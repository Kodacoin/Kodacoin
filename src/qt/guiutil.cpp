...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("kodacoin"))
        return false;
...
{
    // Convert kodacoin:// to kodacoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("kodacoin://"))
    {
        uri.replace(0, 12, "kodacoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Kodacoin.lnk";
}
...
{
    return GetAutostartDir() / "kodacoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Kodacoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  kodacoin-qt [" + tr("command-line options") + "]                     " + "\n";
